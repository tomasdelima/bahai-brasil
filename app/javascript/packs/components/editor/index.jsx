...
export default class Editor extends React.Component {
  ...
  wrapCursor (before, after) {
    ...
    var start, end
    [start, end] = [e.selectionStart, e.selectionEnd].sort((a, b) => a > b)
    var selected = e.value.substring(start, end)
    var newBody = e.value.substring(0, start) + before + selected + after + e.value.substring(end)
    this.props.updateBody(newBody)
    e.value = newBody
    e.focus()
    e.selectionStart = start + before.length
    e.selectionEnd = end + before.length
  }
  ...
  readSelectedLines () {
    ...
    return {start, end, selected: e.value.substring(start, end), previousLines, nextLines, selectedLines, lines}
  }
  ...
  wrapSelectedLines (before, after) {
    ...
    e.focus()
    e.selectionStart = start + before.length
    e.selectionEnd = end + before.length + (before.length + after.length) * (lines.selectedLines.length - 1)
  }
  ...
  captureKeys (e) {
    ...
    if (e.key == "Enter" && this.readSelectedLines().selected.length == 0) {
  ...
  renderButton (label, alt, before, after, wrapSelectedLines) {
    return <Button wrapMethod={wrapSelectedLines ? this.wrapSelectedLines : this.wrapCursor} editor={this.refs.editor} label={label} alt={alt} before={before} after={after}/>
  }
  ...
  render () {
    ...
    return <div>
      ...
      <textarea ref="editor" onChange={this.props.updateBody} value={this.props.body} style={[s.wide("calc(100% - 6px)"), s.high(300), s.margin(10, 0), {resize: "vertical"}].merge()}/>
    </div>
  }
}
