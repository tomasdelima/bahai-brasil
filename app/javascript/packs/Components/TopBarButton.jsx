...
export default class TopBarButton extends Optimized {
  ...
  render () {
    return <Link to={this.props.to} style={s.noDecoration}>
      <Flex white margin={5} radius={4} size={15} pointer bold BG={t[this.props.background] || "transparent"} style={[s.padding(10, 20), s.animate('all', 200), {fontFamily: 'Roboto', color: this.state.color}]} onMouseEnter={this.toggleColor} onMouseLeave={this.toggleColor}>{this.props.title}</Flex>
    </Link>
  }
}
